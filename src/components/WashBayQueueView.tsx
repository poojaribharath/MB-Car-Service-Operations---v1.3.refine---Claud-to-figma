...
interface WashBayQueueViewProps {
...
export const WashBayQueueView: React.FC<WashBayQueueViewProps> = ({
  resource,
  cars,
  onAdvanceQueue,
  onRemoveFromQueue,
  ...
  // Compute wash progress dynamically
  const washJob = activeCar?.jobs.find(j => {
    const t = j.title.toLowerCase();
    return t.includes('wash') || t.includes('detail');
  });
  const activeElapsed = washJob ? washJob.elapsedMins : 8;
  const activeDuration = washJob ? washJob.durationMins : 15;
  const progressPct = Math.min(100, Math.round((activeElapsed / activeDuration) * 100));

  const washCycleTime = 15; // standard washing time

  // Resolve assigned technician name
  const bayTechnician = STAFF_ROSTER.find(t => t.id === resource.assignedTechnicianId)?.name || 'Wash Bay Tech';
  ...
  return (
    ...
    <div className="flex-grow p-4 sm:p-6 pt-2 space-y-6 overflow-y-auto select-none w-full">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-neutral-200 pb-3">
        <div>
          <span className="text-[10px] uppercase font-mono font-bold text-primary-700 bg-primary-50 px-2 py-0.5 rounded tracking-widest inline-block font-sans">Wash Bay Operations</span>
          <p className="text-xs text-neutral-500 font-medium font-sans mt-0.5">
            Evaluate final stage preparation speeds, monitor cleaning throughput, and enforce delivery SLA commitments.
          </p>
        </div>

        {/* Centralised stats metrics bar */}
        <div className="flex flex-wrap items-center gap-3 sm:gap-6 bg-white border border-neutral-200 p-3 rounded-xl text-xs font-mono font-extrabold text-neutral-500 custom-shadow-sm">
          <div>
            <span>QUEUE:</span> <span className="bg-primary-100 text-primary-800 border border-primary-200 rounded-lg px-2 py-0.5 ml-1 font-mono">{waitingCars.length} CARS</span>
          </div>
          <div>
            <span>AVG CYCLE:</span> <span className="text-neutral-900 bg-neutral-100 px-2 py-0.5 border border-neutral-200 rounded-lg ml-1 font-mono">{washCycleTime} MINS</span>
          </div>
          <div>
            <span>BLOCKED EST:</span> <span className="text-neutral-900 bg-neutral-100 px-2 py-0.5 border border-neutral-200 rounded-lg ml-1 font-mono">{waitingCars.length * washCycleTime} MINS</span>
          </div>
        </div>
      </div>
          ...
          {activeCar ? (
            <div className="bg-white rounded-xl p-5 border border-neutral-250 space-y-5 text-xs font-sans custom-shadow-sm">
              <div className="flex items-start justify-between border-b pb-2">
                <div>
                  <span className="font-extrabold font-mono text-neutral-900 text-sm tracking-wider bg-neutral-100 px-1.5 py-0.5 rounded border border-neutral-200 inline-block">{activeCar.plateNumber}</span>
                  <span className="text-[10px] text-neutral-400 capitalize block mt-1.5 font-bold">{activeCar.make} {activeCar.model}</span>
                </div>
                <span className="bg-primary-100 border border-primary-200 text-primary-800 px-2 py-1 text-[8.5px] font-extrabold uppercase rounded-lg tracking-wider animate-pulse">WASHING</span>
              </div>
              ...
              <div className="space-y-2">
                <div className="flex justify-between items-center bg-neutral-50 p-2 rounded-lg">
                  ...
                  <span className="font-bold text-neutral-705 font-mono text-[11px]">{bayTechnician}</span>
                </div>
                <div className="flex justify-between items-center bg-neutral-50 p-2 rounded-lg">
                  <span className="text-neutral-450 font-bold uppercase text-[9px] tracking-wider">Promised Time:</span>
                  <span className="font-bold text-neutral-705 font-mono text-[11px]">{activeCar.promisedTime}</span>
                </div>
                <div className="flex justify-between items-center bg-neutral-50 p-2 rounded-lg">
                  <span className="text-neutral-450 font-bold uppercase text-[9px] tracking-wider">Cycle Countdown:</span>
                  <span className="font-bold text-neutral-750 font-mono text-[11px]">{activeElapsed} / {activeDuration} mins elapsed</span>
                </div>
              </div>
              ...
              >
                <ThumbsUp className="w-4 h-4 text-emerald-300" /> Qualify Clean & Release
              </button>
            </div>
          ) : (
            <div className="bg-neutral-100/30 rounded-xl p-10 border border-dashed border-neutral-250 text-center text-xs text-neutral-450 italic">
              Sprayer cabin empty and ready for queue dispatch loading.
            </div>
          )}
        </div>

        {/* Right Side: Wash Backlog Timeline queue list */}
        <div className="lg:col-span-8 space-y-4">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold font-sans text-neutral-500 uppercase tracking-widest">
              Sequence Backlog ({waitingCars.length})
            </span>
          ...
          <div className="space-y-3">
            {waitingCars.map((car, idx) => {
              const estimatedCompletion = (idx + 1) * washCycleTime;
              let riskBadge = 'bg-neutral-105 text-neutral-500 border border-neutral-250';
              ...
              return (
                ...
                >
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-right">
                    <div>
                      <span className="text-[9px] text-neutral-400 block font-bold uppercase tracking-wider font-mono">Completion ETA</span>
                      <span className="font-bold text-neutral-800 font-mono">In {estimatedCompletion} mins</span>
                    </div>
                    <div>
                      <span className="text-[9px] text-neutral-400 block font-bold uppercase tracking-wider font-mono">Delivery Promise</span>
                      <span className="font-bold text-neutral-800 font-mono">{car.promisedTime}</span>
                    </div>
                    <div className="hidden md:block">
                      <span className="text-[9px] text-neutral-400 block font-bold uppercase tracking-wider font-mono">SLA Assessment</span>
                      <span className={`px-2 py-0.5 rounded text-[9px] font-semibold tracking-wider ${riskBadge}`}>
                        {car.overallStatus === 'critical' ? 'HIGH RISK BREACH' : 'NOMINAL'}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    {/* Queue priorities swap controls */}
            ...
            {waitingCars.length === 0 && (
              <div className="p-8 text-center text-xs text-neutral-400 bg-neutral-100/10 border border-dashed border-neutral-250 rounded-xl italic font-sans animate-pulse">
                Delivery lane empty. Preparing to service upcoming bay wash drop-offs.
              </div>
            )}
          </div>
        </div>
